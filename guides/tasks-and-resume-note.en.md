...
The panel is fully usable without the skills — add, change status, delete, edit in place, open in an external editor; nothing is missing. What you lose is the AI side: without the skills, Claude doesn't know the format, so it can't open tasks or write the resume note, and the "next step" column stays empty.

## File formats

### A task: `.fledge/tasks/NN-short-name.md`

One file per item. No database, no index file — the panel and the AI both read and write the markdown files directly.
...
Field names and values are always in English — that's the format contract between the panel and the skill. The title and body can be in any language.

**Numbers can collide.** The panel and the AI each assign numbers ("highest + 1"), so creating tasks from both at the same time can produce the same number. When that happens, the panel shows both, flags both, and you rename one of the files by hand. Not adding locking is deliberate: a coordination mechanism for a rare case isn't worth the complexity.

### The resume note: `.fledge/state.md`

Overwritten, not appended — it records where things stand now, not history. History is what git log is for.
...
The full form (for when a phase is done and the next hasn't started) adds five sections: what to load, what state to confirm, the full procedure, working conventions, and things a fresh session needs to know. The template is in the skill.
...
Claude Code's built-in memory system also responds to "remember this", and it outranks any skill. Say just "remember this" and the memory system usually takes it — `fledge-tasks` never even loads. The dividing line:
...
This came up for real during packaging acceptance; it isn't a guess.
...
With multiple accounts (multiple config directories), point the other accounts' `skills/` at the same directory so every skill only has to be installed once:
...
- **It doesn't decide what's worth keeping.** The AI never opens a task on its own — if you didn't ask, it doesn't. Deciding what's worth recording is your job.
- **It doesn't delete tasks.** Done means `status: done`; the file stays. Analysis that vanishes the moment it's finished is exactly what this is designed to prevent.
...
- **It doesn't install skills from inside the app.** Copy them yourself.

## Appendix: `handoff`

`resume-note` is for wrapping up; `handoff` is for "context is running out, continue in a new conversation". It calls `resume-note` for the full form, then records a test baseline, checks the branch state, lists memory candidates, and finally hands you three sentences to paste into the new conversation.

It's tied to the author's own workflow (spec → plan → task-by-task execution, a progress ledger, Codex adversarial review) and may not fit yours as-is. It ships in `skills/handoff/` as a reference.
