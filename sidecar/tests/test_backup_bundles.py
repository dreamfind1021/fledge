...
def test_partial_and_unrelated_names_are_ignored(tmp_path: Path):
    ...
    _touch(tmp_path, ".claude-backup-20260729-0900-12345-678.tar.gz.partial")
...
def test_last_attempt_not_failed_when_partial_older(tmp_path: Path):
    """舊殘骸不該讓一次成功的備份看起來像失敗。"""
    _touch(tmp_path, ".claude-backup-20260727-1000-12345-678.tar.gz.partial")
    _touch(tmp_path, "claude-backup-20260729-0900.tar.gz")
...
def test_partial_without_pid_segment_is_not_our_residue(tmp_path: Path):
    """命名不符的檔案不是本腳本產生的格式，不能拿它宣稱備份失敗。

    **本條只驗 Python 這一半。** 腳本回收路徑的對應主張在
    `test_backup_shell.py::test_reclaim_only_deletes_our_exact_naming`——先前把兩個主張
    都寫在這裡的 docstring 裡，但測試從沒跑過 shell，是假綠。"""
    _touch(tmp_path, ".claude-backup-20260729-0900.tar.gz.partial")        # 缺 PID/隨機段
    _touch(tmp_path, ".claude-backup-20260729-0900-12345.tar.gz.partial")  # 舊格式
    _touch(tmp_path, ".claude-backup-20260729-0900-abc-def.tar.gz.partial")
    assert last_attempt_failed(str(tmp_path), []) is False
