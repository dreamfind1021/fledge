...
def _restore_gate(
    ...
    回**判別碼字串**代表擋下，回 `(bundle_abs, dest_abs)` 代表放行——**放行時回的就是它驗過
    的那一組值，呼叫端直接拿去組 argv，不要再解析一次**（Codex 票 11 審查）。閘裡算一組、
    組 argv 時又算一組的話，實際跑的可能是**沒被 `check_dest` 驗過**的另一個位置：
    `resolve_dest` 的撞名迴圈會探測檔案系統，兩次之間狀態變了就給出不同答案；而重解析本身
    也可能拋 `ValueError` 變成裸 500（例如隨身碟在這中間被拔掉）。
    ...
    **來源的二擇一在這裡多判一次**（`resolve_source_and_dest` 內也有）：不是為了規則，而是
    ...
    return (bundle_abs, dest_abs) if verdict == "ok" else f"dest_{verdict}"
...
def create_session(req: CreateSessionRequest):
    ...
    if req.kind == "restore":
        ...
        if isinstance(gate, str):
            return JSONResponse(status_code=400, content={"error": gate})
        # argv 由後端組，**用閘剛驗過的那一組值**（不重新解析，理由見 `_restore_gate`）。
        # 名字模式仍走 list_bundles 的 allowlist（沿用 kind=install 只送 install_id 的
        # 不變式）；路徑模式的鬆綁與其理由見 `restore.bundle_from_path`。
        return _unattributed_session(restore.build_restore_argv(*gate), req.path)
