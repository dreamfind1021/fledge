...
Fledge is a development and project-management tool for vibe coders. The development half runs the real Claude Code against any of your projects, all in one window. The project-management half isn't the kind that assigns work to a team — it's built for the **back-and-forth between you and an AI**.

It organizes everything around "project × AI session": one click opens a real Claude Code session with the right account in the right folder. Before you start, you see where you left off and what's still open; decisions have somewhere to live; the connections between projects are visible. No terminal ritual, no IDE clutter — and no amnesia just because the interface is simple.

Under the hood it drives **the real Claude Code on your machine** — no rewrite, no proxy. Your existing skills, `CLAUDE.md`, MCP servers, and signed-in accounts carry over untouched. Fledge adds a thin interface on top — and three layers of memory.
...
Everything it grew afterwards was a response to some kind of forgetting. Losing track of where you left off when switching projects led to the resume note. "Things to deal with later" piling up in a hand-written `.md` led to the task panel. Research and ideation notes scattered across folders with nothing tying them together led to the second-brain structure and the memory panel. The launcher removes friction at the start; the other three remove friction when you come back — and in use they feel like short-term, mid-term, and long-term memory: the resume note, the tasks, the second brain. None of this was designed up front; it grew out of use.

The author is a vibe coder — someone who builds with AI, without a software-engineering background. When they asked an engineer how they manage projects, the answer was the system their engineering team uses — one designed for dividing work among people. What needed managing here was the back-and-forth between a person and an AI. Fledge is built for that.
...
| Traditional project-management tools | Those manage how work is divided among people; Fledge manages the back-and-forth between you and an AI — where you stopped, what's next, where decisions go |
...
The section above talks about short-, mid-, and long-term memory because that's how it feels in use. In the design, what separates the layers isn't duration but **who decides what gets written, and how far it reaches**:
...
| **What you ask the AI to remember** | Where you stopped, what's next | You ask, the AI writes | `.fledge/state.md`, `.fledge/tasks/` | One project; the panel shows every project |
...
A common complaint about AI is "it doesn't remember what we were doing". With these three layers in place, that complaint mostly goes away — not because the model changed, but because every kind of thing worth remembering has a fixed place, and the AI can reach it when work starts: `CLAUDE.md` and its own memory load on every launch; the resume note and tasks live in the project, and "read `.fledge/state.md` first" as the opening line picks them up; a hook feeds it the second brain whenever it starts a session in that vault. The memory isn't in the model. It's in files.

### What the AI remembers on its own

Fledge doesn't touch this layer at all. Claude Code, Codex, and the other CLIs already have their own memory: `CLAUDE.md` holds the rules you give it; per-project memory holds the notes it writes for itself. That layer already exists and works well; there's no reason to redo it. What Fledge adds is one screen that lays out these notes for **every project and every account** (see the memory panel below), because the CLI can't see the whole picture by itself.
...
- **Project sidebar** — scans the projects under your roots, groups them by account with a color per account, so work and personal stay apart at a glance. Expand a project's file tree in the sidebar and drag a file into the terminal to paste its path.
...
- **The real Claude Code** — the embedded terminal (xterm.js + a PTY bridge) runs the actual Claude Code. Conversations, skills, MCP, permission prompts: exactly what you'd see in a terminal. The parts that tend to break in xterm — CJK input methods, copy/paste, the context menu — have all been fixed.
...
- **Onboarding wizard** — environment detection, one-click installs, account sign-in, shared setup across accounts, template deployment. Anything you skip can be finished later from Settings.
...
Fledge **does not install any of these automatically** — it never touches `~/.claude/settings.json`, `CLAUDE.md`, or `~/.claude/skills/`. What to install, if anything, is up to you.
...
| `handoff` | When context is running out and you need to continue in a fresh conversation: writes the full resume note and produces a prompt to paste into the new one | No effect |
...
`handoff` is tied to the author's own workflow (superpowers' spec → plan → task-by-task execution). It may not fit your workflow as-is; treat it as a reference. The skills themselves are written in Traditional Chinese.
...
The worst part of installing someone else's tool is wondering what it quietly changed. Here is exactly where Fledge's boundaries are:

- **It doesn't write your Claude Code settings.** `~/.claude/settings.json`, `CLAUDE.md`, `skills/` — none of them. You copy skills yourself. The wizard's "shared setup" is the one exception: it lists exactly what it's about to create, and creates the symlinks only after you confirm.
...
- **Child processes never see Fledge's token.** The sidecar's auth token is not passed into Claude Code's environment.
...
- If closing a session fails, the UI treats it as closed and loses track of the leftover Claude Code process.
...
- The "Reveal in Finder" capability has no path scope — it's broader than it needs to be.
