...
| `i18n.ts` | i18n 最小基建（react-i18next）：語言權威序 localStorage `fledge-lang`（讀取包 try/catch，Storage 拋錯退回 OS 偵測——這行在 module 層跑，拋出去＝app 開不起來）> OS zh 偵測 > en；**注意 `docs/planning/i18n-design.md` §3 原設計是 config `ui.language` 為真相源，該欄位從未實作，對齊工作見 `.scratch/i18n-language-truth-source/`**；defaultNS dashboard；註冊 namespace `dashboard` + `memory` + `sidebar` + `onboarding`；init 與 `languageChanged` 同步 `document.documentElement.lang`（`index.html` 是靜態 `lang="en"`） | default `i18n` |
...
| `locales/{zh-TW,en}/onboarding.json` | 引導精靈與設置卡 locale catalog（common/welcome/roots/env/login/cc/sys/done/st 九群共 89 key；文案取自 `docs/design/b4-onboarding-wizard.html` 的 `T` 物件）。依 i18n-design §2.3 只放語意標記與插值，樣式一律留在 CSS——`<Trans>` 標籤僅 `<strong>`/`<code>`/`<br />`（basic nodes，見 `i18n.ts`）與具名 `<rerun>`（票 29 映射成按鈕）；插值變數 `{{source}}`/`{{target}}`（帳號）、`{{path}}`/`{{name}}`（路徑或檔名）、`{{count}}`/`{{projects}}`/`{{roots}}`/`{{accounts}}`（計數）。`onboarding-parity.test.ts` 鎖三件事：兩語 key 集合一致、無 inline style/`<span>`、同 key 的插值與標籤兩語一致 | — |
...
| `components/LangSwitch.tsx` | 語言切換膠囊（中文／English autonym 不進 catalog）：`await i18n.changeLanguage` 成功**才**寫 localStorage `fledge-lang`（i18n 權威序最高階），兩步各自 try/catch——切換失敗不動快取、Storage 失敗只是記不住不擋切換；`aria-pressed` 標目前語言。定位交給呼叫端容器，設定頁 header 已掛，歡迎頁於票 22 掛上 | `LangSwitch` |
