...
| `components/CommonConfigCard.tsx` | 精靈共通設置頁（票 26）：source＝第一個登記帳號（實體檔持有者，精靈不讓使用者改選）、target＝其餘帳號中 `checkDir` 回 `dir` 者——**目錄未確認存在的一律排除**（`apply` 會 `mkdir` target dir，照送等於替單帳號使用者建出他沒要的目錄），全部排除或只登記一個帳號時整張卡「不適用」灰態（`.b4-card.is-na`）；逐項顯示現況（`STATE_TEXT` 窮舉後端 11 個 state）與將執行的動作（`chipFor`：`needs_overwrite` 先攔成「保留不動」琥珀、`skip` 但非 `ok` 顯示「無法處理」而不是「已就緒」），多 target 逐帳號分組；**apply 一律送空 `overwrite`**（spec-b4 定案 8，逐項授權在票 29 的設定頁版）、有 `needs_overwrite` 時顯示指向設定頁的琥珀說明、進階項 `projects` 只給一行說明；套用後 chip 換成逐項結果（`results.*`）並**重新 plan**（狀態即時偵測，spec-b4 §4）、主按鈕讓位給「下一步」；判別碼經 `MAPPED_ERRORS` 映射、未映射者退 `errors.common_config_failed`（判別碼與逐項 `failed` 的 error 只進 console）；`latest-request-wins` 走 `reqId`（同 EnvCard），effect dep 用 `accountsSig`（父層每 render 換 accounts 引用） | `CommonConfigCard` |
...
| `sidecar/.../routes/setup.py`／`setup/*.py`（偵測 payload/資料表／共通設置／範本合約變更） | `src/lib/sidecar.ts`（`ToolStatus` 型別與 `fetchSetupStatus`；共通設置的 state/action/outcome union 與 `COMMON_CONFIG_ENTRIES`）、`src/components/EnvCard.tsx`（依 `tier`／`install_command`／`manual_command` 決定列什麼）、`src/components/CommonConfigCard.tsx`（`STATE_TEXT`／`ACTION_CHIP`／`OUTCOME_TONE` 三張窮舉表與 `MAPPED_ERRORS`；後端加 state／action 會編譯失敗，加判別碼則靜默退通用訊息——需一併補 catalog）、`CommonConfigCard.test.tsx`、`EnvCard.test.tsx`、`test_setup_routes.py`、`test_env_detect.py`、`test_install_specs.py`、`test_common_config.py`、`test_templates.py`、`test_template_manifest.py` |
