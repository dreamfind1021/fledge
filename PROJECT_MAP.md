...
| `components/SystemSettingsCard.tsx` | 精靈系統設置頁（票 27）：訂閱服務清單（名稱＋月費，可增刪列）與知識庫根目錄（輸入框＋picker）兩區，範本卡待票 28。**兩區刻意沒有各自的儲存鈕**（視覺權威 demo 的定案）：頁尾只有「全部略過」與「下一步」，於是「下一步」＝送出使用者真的**改過**的區塊、「全部略過」＝什麼都不送就前進；兩區都沒填自然等於整段略過。沒改就不送（`sameSubscriptions` 比訂閱、字串比 `kms_root`）——用一次可能失敗的請求換零變更只會擋住想直接往下走的人，且「既有值被刪光」仍會送出空清單而非靜默吞掉；空白列走 `dropBlankRows`（只填一欄的仍由 `validateSubscriptions` 擋成欄位錯誤）；兩段各自的失敗以 `stage` 記錄後映射成 `errors.subs_failed`／`errors.kms_failed`，訂閱已存進去就不會謊稱它失敗；`port` 為 null 時**有待送出的改動就擋下**（`errors.backend_unavailable`）——默默前進會讓剛填的東西憑空消失，沒改動則照樣可以往下走；KMS 存完回讀 config（比照 `Settings`），回讀失敗只 warn 不當成儲存失敗；初值取自 config props 不再同步（走到本頁時 config 早已落檔，重跑引導帶既有值進來） | `SystemSettingsCard` |
