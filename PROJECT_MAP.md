...
| `components/Onboarding.tsx` | 首次設定全屏 wizard（is_first_run 觸發）：七頁容器＝歡迎／根目錄／環境／登入／共通設置／系統設置／完成，序列與進度條走 `lib/onboardingSteps`（單帳號六頁）；**根目錄頁落檔後留在精靈**（ADR-0003：`onboard` first-run only，之後各頁走細粒度端點），落檔狀態記在元件 state（不讀 `config.is_first_run`——後續 config write 會使該欄變 undefined，Codex F-7），落檔後根目錄清單轉唯讀、主按鈕只前進不重打 onboard；換頁清暫態訊息並捲回頂端；歡迎頁掛 `<LangSwitch>`；環境頁掛 `<EnvCard>`（票 23，含該頁標題與導覽），登入／共通設置／系統設置三頁目前只有標題與導覽，內容待票 25–28；依 scan-preview status 擋無效 draft、denied 用 amber notice；字串全走 `t()`（onboarding ns） | `Onboarding` |
...
| `src/components/Terminal.tsx`（onmessage / teardown 變更） | §7 終端機不變式、`src/lib/activityTracker.ts`、`src/components/EnvCard.tsx`（第二個掛載點：卡片內安裝終端機，走合成 tabId、store 內無對應 tab） |
| `src/lib/sidecar.ts` 的 `createSession`（簽名／body 欄位變更） | `src/store/useAppStore.ts`（openTab）、`src/components/EnvCard.tsx`（kind=install）、`sidecar.test.ts`、`useAppStore.test.ts`、`sidecar/.../routes/sessions.py` 的 `CreateSessionRequest`（`extra="forbid"`，欄位不對即 422） |
