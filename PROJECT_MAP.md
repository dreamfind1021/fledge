...
| `routes/usage.py` | `GET /usage/dashboard?days=`（觀測儀表板單一端點）：single-flight 掃描（25s stale gate、error 立即 retry、days 變更視為 stale）＋`asyncio.to_thread` 不卡 event loop＋last-good 語義（掃描中回舊資料標 scanning、冷掃 202+Retry-After、冷掃失敗 error-only 200）；**逐 `UsageEntry` 按活動 log 區間歸屬**（`account_activity.load_sessions`(帶 bridge live 集合)+`attribute`，僅絕對 cwd）組 `claude_entries_by_account`，涵蓋不到 fallback 到 scanner canonical 並標 `partial`；模組級 `_state`（snapshot/cache/scanning） | `router`, `reset_state_for_tests()` |
...
| `usage/aggregator.py` | dashboard payload 組裝：claude dedup 替換規則（parent>sidechain>token 多者）、synthetic 濾除、KPI（本地時區視窗/Net ROI/分源 cache 命中率）、daily/models/projects（memoized realpath）/hourly（7×24 週一起）聚合（900s bucket memo 消 datetime 成本）、5hr blocks per 帳號各自 dedup 後建（全量建後濾 7 天，payload `blocks.claude.accounts[]`，每帳號帶 `partial`；不帶 `claude_entries_by_account` 走 fallback 舊 shape） | `build_dashboard()` |
...
| `components/Dashboard.tsx` | 觀測儀表板分頁（單頁長滾動）：30s 輕輪詢（首掃 2s、visibility/isActive gating、error 保留舊資料標 stale）＋六面板（KPI 5 卡/Claude 5hr 窗口 per 帳號條（樣本足顯進度條、不足顯用量+burn+reset；含 fallback 歸屬的帳號標「部分推估」badge）＋Codex 官方雙 gauge/每日成本堆疊長條/模型 donut/時段 heatmap 週一起/專案表 top30）；全手刻 SVG/CSS 零圖表依賴、字串全走 t() | `Dashboard` |
...
| `components/TabBar.tsx` | tab 列：切換 + 帳號 chip + 關 tab + 統一狀態點（`tabDotState`：working 呼吸/waiting 穩定/連線態，取代 ●/⚠ 前綴）；terminal 分頁以 SquareTerminal 圖示取代狀態點；memory 分頁以 `Brain` 圖示 + 標題走 `memory:tabTitle`；分頁改 `SortableTab`（dnd-kit `useSortable` 水平排序；close button `onPointerDown` stopPropagation 不啟動拖曳）；aria 走 sidebar ns | `TabBar` |
...
| `components/Workspace.tsx` | TabBar+Terminal 合成一體面板（保留全 tab mount + display 切換）；tab 狀態矩陣與 ended/offline 子狀態；kind 分派加 `memory`→`<Memory>` 分支；Terminal 分支（claude/terminal + projectPath）內掛 `<RelatedFloat>`（既有 absolute wrapper 為定位包含塊，不改終端機尺寸）；`ws-term-area` 設 `useDroppable`（id `terminal-drop`，需求 4 drop target） | `Workspace` |
...
| `sidecar/.../usage/account_activity.py`（事件格式/歸屬規則變更） | `routes/sessions.py`（寫 open/close）、`routes/usage.py`（讀 load_sessions+attribute）、`pty_bridge.py`（live_ids/on_close 契約）、`test_account_activity.py` |
