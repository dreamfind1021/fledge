...
| `locales/{zh-TW,en}/onboarding.json` | 引導精靈與設置卡 locale catalog（common/welcome/roots/env/login/cc/sys/done/st/errors 十群共 97 key；文案取自 `docs/design/b4-onboarding-wizard.html` 的 `T` 物件，`errors.*` 為 spec-b4 §5 約定的判別碼群〔key 用判別碼原名〕）。依 i18n-design §2.3 只放語意標記與插值，樣式一律留在 CSS——`<Trans>` 標籤僅 `<strong>`/`<code>`/`<br />`（basic nodes，見 `i18n.ts`）與具名 `<rerun>`（票 29 映射成按鈕）；插值變數 `{{source}}`/`{{target}}`（帳號）、`{{path}}`/`{{name}}`（路徑或檔名）、`{{count}}`/`{{projects}}`/`{{roots}}`/`{{accounts}}`（計數）。`onboarding-parity.test.ts` 鎖三件事：兩語 key 集合一致、無 inline style/`<span>`、同 key 的插值與標籤兩語一致 | — |
...
| `lib/onboardingSteps.ts` | 純函式：引導精靈頁面序列（七頁；帳號數 <2 時整個 `common` 頁不出現＝六頁）＋索引夾取（兩端停住不繞回，吸收序列縮短後的超界索引）＋進度條格數與填色（0-based，填到目前頁為止） | `wizardSteps()`, `clampStepIndex()`, `progressCells()`, `WizardStep` |
