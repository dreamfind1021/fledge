...
| `components/Onboarding.tsx` | 首次設定全屏 wizard（is_first_run 觸發）：七頁容器＝歡迎／根目錄／環境／登入／共通設置／系統設置／完成，序列與進度條走 `lib/onboardingSteps`（單帳號六頁）；**根目錄頁落檔後留在精靈**（ADR-0003：`onboard` first-run only，之後各頁走細粒度端點），落檔狀態記在元件 state（不讀 `config.is_first_run`——後續 config write 會使該欄變 undefined，Codex F-7），落檔後根目錄清單轉唯讀、主按鈕只前進不重打 onboard；換頁清暫態訊息並捲回頂端；歡迎頁掛 `<LangSwitch>`；環境頁掛 `<EnvCard>`、登入頁掛 `<LoginCard>`、共通設置頁掛 `<CommonConfigCard>`（三者都含該頁標題與導覽），系統設置頁目前只有標題與導覽，內容待票 27–28；依 scan-preview status 擋無效 draft、denied 用 amber notice；字串全走 `t()`（onboarding ns） | `Onboarding` |
...
| `src/lib/onboardingSteps.ts`（頁面序列增刪／降級規則變更） | `Onboarding.tsx`（頁面渲染分支與 `PENDING_NS` 對照）、`onboardingSteps.test.ts`、`Onboarding.test.tsx`、`.scratch/onboarding-wizard/spec-b4.md` 定案 9 |
