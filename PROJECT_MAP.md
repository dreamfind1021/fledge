...
| `locales/{zh-TW,en}/onboarding.json` | 引導精靈與設置卡 locale catalog（common/welcome/roots/env/login/cc/sys/done/st/results/errors 十一群共 162 key；文案取自 `docs/design/b4-onboarding-wizard.html` 的 `T` 物件，`errors.*` 與 `results.*` 為 spec-b4 §5 約定的判別碼群〔key 用判別碼原名，`cc.state.*`／`sys.state.*`／`sys.result.*` 同此約定；範本的逐檔結果**不共用 `results.*`**——那組措辭是共通設置的「已建立連結」，對範本是錯的〕，`sys.tpl.*` 以**範本 id** 為 key（demo 的位置式 `tpl1`–`tpl3` 已汰換，後端 `label`／`description` 只在 catalog 沒有該 id 時當 fallback）；demo 的 `cc` 四條現況文案只覆蓋靜態示意的 4 個狀態，票 26 改成窮舉後端 11 個 state 的 `cc.state.*`）。精靈的三段品牌字串（`Fledge` wordmark／tagline／`Built for Claude Code`）刻意不進 catalog、兩語同顯英文，屬 §4.6.13 記錄例外（demo 同樣未給 `data-t`）。依 i18n-design §2.3 只放語意標記與插值，樣式一律留在 CSS——`<Trans>` 標籤僅 `<strong>`/`<code>`/`<br />`（basic nodes，見 `i18n.ts`）與具名 `<rerun>`（票 29 映射成按鈕）；插值變數 `{{source}}`/`{{target}}`（帳號）、`{{path}}`/`{{name}}`（路徑或檔名）、`{{count}}`/`{{projects}}`/`{{roots}}`/`{{accounts}}`（計數）。`onboarding-parity.test.ts` 鎖三件事：兩語 key 集合一致、無 inline style/`<span>`、同 key 的插值與標籤兩語一致 | — |
