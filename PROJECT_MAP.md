...
| `test_pty_bridge.py` | PTY round-trip、env override、env_remove 剔除、並發、`close()` 失敗時的 handle 去留（仍活著留、已死或問不出狀態丟） |
