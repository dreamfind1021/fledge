...
PY
  ...
  if [ -n "${hit}" ]; then
    ...
    # 驗收流程本來就要求把這個目錄加成帳號（不然卡片看不到它），所以撞到這道守門是常態，
    # 不是使用者做錯什麼——訊息必須直接給出路，否則驗收會卡在這裡（2026-07-27 實測）。
    echo >&2
    echo "這是預期中的守門（本腳本不碰 Fledge 正在用的帳號目錄）。出路：" >&2
    echo "  ① 到設定頁移除帳號「${hit}」→ 重跑本指令 → 需要的話再把帳號加回來" >&2
    # 「換個路徑另造一份」只對造狀態的模式成立：--clean 換路徑只會去清一個通常不存在的
    # 目錄，等於叫使用者做白工（Codex R4 指出）
    if [ "${MODE}" != "--clean" ]; then
      echo "  ② 換一個路徑另造一份：FLEDGE_PROBE_DIR=~/.claude-probe2 '$0' ${MODE}" >&2
    fi
    exit 1
  fi
}
